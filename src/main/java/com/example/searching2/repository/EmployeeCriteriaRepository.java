package com.example.searching2.repository;
...
public class EmployeeCriteriaRepository {
    ...
    private Predicate getPredicate(EmployeeSearchCriteria employeeSearchCriteria,
        ...
        if (Objects.nonNull(employeeSearchCriteria.getFistName())) {
            predicates.add(
                    criteriaBuilder.like(employeeRoot.get("fistName"),
                            "%" + employeeSearchCriteria.getFistName() + "%")
            );
        }
        if (Objects.nonNull(employeeSearchCriteria.getLastName())) {
            predicates.add(
                    criteriaBuilder.like(employeeRoot.get("lstName"),
